...
def clean_data():
	"""Load, clean, and preprocess the lung cancer survey data.
	
	Returns:
		pd.DataFrame: Cleaned DataFrame exported to cleaned_survey_lung_cancer.csv
	"""
	#loading the data
...
def train_and_predict():
	"""Train the ML pipeline and make predictions on sample data.
	
	Loads cleaned data, trains a Random Forest model with hyperparameter tuning,
	evaluates performance, and makes a prediction on sample patient data.
	"""
...
# if __name__ == '__main__': this is to ensure that the main functions are only executed when the script is run directly, and not when it is imported as a module in another script.
